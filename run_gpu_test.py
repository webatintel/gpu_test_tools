...
def parse_arguments():
  ...
  parser.add_argument('target',
      choices=['webgl', 'webgl2', 'dawn', 'angle', 'gpu', 'aquarium'],
  ...
  parser.add_argument('--dir', '-d', default='.',
      help='Chrome/Dawn/ANGLEAquarium directory.\n\n')
  ...
  elif args.target == 'dawn':
    if not args.backend in ['blink', 'end2end', 'end2end_wire', 'end2end_validation', 'perf']:
      raise Exception('Unsupported backend: ' + args.backend)
  ...
  if args.filter:
    if (args.target == 'aquarium'
        or (args.target == 'dawn' and args.backend == 'blink')
        or (args.target == 'angle' and args.backend == 'end2end')):
      raise Exception('Do not support filter for %s/%s' % (args.target, args.backend))
...
def generate_webgl_arguments(args):
  ...
  if args.target == 'webgl2':
    total_args.extend(['--webgl-conformance-version=2.0.1',
  ...
  browser_args.extend(config['tryjob_args']['webgl_'+args.backend])
...
def generate_blink_arguments(args):
  ...
  if is_win():
    total_args.extend(['--additional-driver-flag=--disable-gpu-sandbox'])
  elif is_linux():
    total_args.extend(['--additional-driver-flag=--use-vulkan=native',
                       '--no-xvfb'])
  return total_args
...
def generate_gtest_arguments(args):
  ...
  if args.backend == 'perf':
    ...
    total_args.extend(['--use-gpu-in-tests',
                       '--test-launcher-retry-limit=0'])
...
def execute_shard(cmd, args):
  ...
  for n in range(1, args.repeat+1):
    ...
    new_cmd = cmd[:]
    if args.target.startswith('webgl') or args.backend == 'blink':
...
def main():
  ...
  elif args.backend == 'blink':
    cmd = [PYTHON_CMD, path.join(args.dir, BLINK_TEST_SCRIPT)]
    cmd.extend(generate_blink_arguments(args))
    total_shards = '--total-shards'
    shard_index = '--shard-index'
  elif args.target == 'aquarium':
